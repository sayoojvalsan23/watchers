...
Last reviewed: 2026-09-03 (box now the full arc, 73-98 E / 26-37.5 N; **Phase 0 ceiling is 2.91 alerts/yr, TUNE. The gate is not passed, and D18/D19 record why it must not be tuned into passing.**)
...
| D18 | **The gate can be passed by tuning, and every passing setting is a trap.** Swept against both halves. The warning threshold does nothing at all -- raising it moves events from warning to advisory and both are dispatch tiers. The levers that DO move the rate move it off a cliff: the dispatch population above 60 is `92:6, 77:28, 62:2`, so a single spike of **28 events at score 77** fails the gate on its own. advisory=75 gives 2.83/yr; advisory=80 gives **0.51/yr and GO**. There is no gradual setting, only a choice of which side of the spike to stand on. `max_depth_km` 10.0 -> 8.0 gives 1.97/yr, a pass by 0.03. | A threshold balanced on a 28-event spike is the mistake `threshold_provenance` already records -- "the founding event scored exactly 75, the threshold sat on top of a data point" -- repeated with a new number. The next event that scores 79 rather than 81 falls off it silently. | **OPEN, and deliberately not taken.** |
| D19 | **The recall half of the paired gate cannot discriminate.** The curated set is `us7000tbwb` and `us7000tc90`: the 26 August collapse and its aftershock four hours later. One event, two records, both depth 0.0, both scoring 95 in the ceiling. Every setting in the D18 sweep reports **2/2**, including `max_depth_km = 1.0`, which would reject almost anything not already at the surface. | The gate is paired precisely so that false-alarm tuning cannot quietly destroy sensitivity -- and the half that is supposed to catch that returns the same answer for every setting. It cannot distinguish "more careful" from "deaf". **On this system, deaf is the failure that kills people.** | **OPEN. This blocks tuning, not the other way round.** Phase 0 cannot be closed by moving thresholds while one event carries the entire recall side. What unlocks it is more curated events, or a discriminator that does not rest on a threshold at all -- the waveform association test separated the founding event from background at 3.2 km/s against physically impossible values, with no threshold and no cliff. |
